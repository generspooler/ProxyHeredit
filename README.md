...
Install will prompt for a GitHub token if none is configured. This avoids GitHub API rate limiting (60 → 5000 requests/hour) for tools like opencode.

Create a token at [github.com/settings/tokens](https://github.com/settings/tokens) (no scopes needed).

### Windows

```powershell
.\install.ps1
# ── Enter GitHub Token (blank to skip): ghp_xxxx
```
...
The token is appended to your shell rc file (`~/.zshrc`, `~/.bashrc`, or PowerShell `$PROFILE`). To change it, run `uninstall.sh` / `uninstall.ps1` first (or remove the existing token line), then re-run install.
...
安装时会询问是否配置 GitHub Token，用于避免 GitHub API 限流（60 → 5000 次/小时），让 opencode 等工具免于 403 错误。

在 [github.com/settings/tokens](https://github.com/settings/tokens) 创建（无需勾选任何权限）：
...
Token 会写入 shell 配置文件（`~/.zshrc`、`~/.bashrc` 或 PowerShell `$PROFILE`）。若要更换，请先运行 `uninstall.sh` / `uninstall.ps1`（或手动删除旧行），再重新运行安装脚本。
