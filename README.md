...
让 Windows 上所有 CLI 工具自动继承系统代理设置——解决 opencode、Claude Code、npm、Go、pip 等工具"有系统代理但走不通"的问题。
...
`proxy-env-bridge` 在 PowerShell 启动时自动执行：
...
```powershell
# 1. 从 Profile 中移除 proxy-env-bridge 代码
